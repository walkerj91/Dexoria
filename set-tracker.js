...
async function init() {
  // ── Render the set list immediately — this needs no auth at all, and
  // must never be blocked by a slow/stuck auth check (see below) ──
  state.collected = loadCollected(); // anonymous key for now; re-keyed below if a user resolves
  populateFromCache();
  renderTabs();
  ...
  // never leave the whole page blank again. ──
  try {
    const user = await Promise.race([
      supabase.auth.getUser().then(r => r.data.user),
      new Promise((_, reject) => setTimeout(() => reject(new Error('auth check timed out')), 4000)),
    ]);
    if (user) {
      window.__dexoriaUserId = user.id;
      // Re-load under the correct per-user key and reflect it in the UI
    ...
    console.warn('[SetTracker] Auth check failed or timed out — continuing as anonymous:', err);
  }
